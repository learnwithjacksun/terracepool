...
const PricesPage: React.FC = () => {
  ...
  const totalPrice = useMemo(() => {
    ...
    return total;
  }, [selectedModelId, selectedAddons]);
  ...
  return (
    <div className="bg-cream text-warm-text font-body antialiased overflow-x-hidden relative min-h-screen">
      ...
      <main className="pt-32 md:pt-36 pb-20">
        <section data-aos="fade-up" className="max-w-7xl mx-auto px-6 md:px-8 mb-10 md:mb-16 text-center lg:text-left">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
            ...
            <div className="lg:col-span-7 space-y-12">
              ...
              <div data-aos="fade-up">
                <h2 className="text-2xl font-bold text-warm-text mb-6 pb-4 border-b border-warm-border/50">{t('pricing.select_model')}</h2>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-3">
                          ...
                          <span className="font-black text-warm-text text-lg">{formatPrice(model.price)}</span>
                        </div>
                        <span className={`font-bold leading-tight ${isSelected ? 'text-warm-text' : 'text-warm-text-light'}`}>
                          {t(model.nameKey).split('(')[0].trim()}
                        </span>
                        <span className="text-xs text-warm-muted font-medium mt-1">
                          {t(model.nameKey).includes('(') ? t(model.nameKey).split('(')[1].replace(')', '') : 'Dimensions TBD'}
                        </span>
                      </div>
                    );
                  })}
              ...
              {pricingCategories.map((category) => (
                <div key={category.categoryId} data-aos="fade-up">
                  <div className="flex items-center justify-between mb-6 pb-4 border-b border-warm-border/50">
                    <h2 className="text-2xl font-bold text-warm-text">{t(category.nameKey)}</h2>
                    {category.type === 'multiple' && (
                      <span className="text-[10px] md:text-xs text-warm-muted font-label uppercase tracking-widest bg-warm-border/10 px-3 py-1 rounded-full">Multiple select</span>
                    )}
                  </div>
                  <div className="space-y-3 md:space-y-4">
                    {category.options.map((opt) => {
                      const isSelected = (selectedOptions[category.categoryId] || []).includes(opt.id);
                      return (
                        <div 
                          key={opt.id}
                          onClick={() => handleOptionToggle(category.categoryId, opt.id, category.type)}
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <span className={`text-sm md:text-base ${isSelected ? 'font-bold text-warm-text' : 'font-medium text-warm-text-light'}`}>
                              {t(opt.nameKey)}
                            </span>
                          </div>
                          <span className="font-bold text-warm-text whitespace-nowrap">+{formatPrice(opt.price)}</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            <div className="lg:col-span-5 relative mt-10 lg:mt-0">
              <div className="lg:sticky lg:top-32 space-y-6">
                ...
                {selectedModel && (
                  <div className="flex flex-col gap-4 animate-fade-in">
                    <div className="rounded-3xl overflow-hidden warm-shadow-lg border-2 border-warm-border/40 relative aspect-video bg-cream-dark group w-full">
                      <img key={`${selectedModel.image}-photo`} src={selectedModel.image} alt="Pool Photo" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                      ...
                      <div className="absolute bottom-5 left-6 right-6 text-white flex justify-between items-end">
                        <div>
                          ...
                          <span className="text-sm font-bold uppercase tracking-widest text-gold drop-shadow-md">
                            {t(selectedModel.nameKey).includes('(') ? t(selectedModel.nameKey).split('(')[1].replace(')', '') : 'Dimensions TBD'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {selectedModel.drawingImage && (
                      <div className="rounded-3xl overflow-hidden warm-shadow border-2 border-warm-border/20 relative aspect-video bg-white">
                        <img key={`${selectedModel.drawingImage}-drawing`} src={selectedModel.drawingImage} alt="3D Model Drawing" className="w-full h-full object-contain p-4" />
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="bg-white rounded-3xl p-6 md:p-8 warm-shadow-lg border border-warm-border/30 relative overflow-hidden">
                  ...
                  <h3 className="text-lg font-bold text-warm-text mb-6 border-b border-warm-border/50 pb-4 relative z-10">{t('pricing.your_configuration', 'Your Setup')}</h3>
                  
                  <div className="space-y-4 mb-8 relative z-10">
                     <div className="flex justify-between items-start gap-4">
                        <span className="text-warm-text-light text-sm font-medium">{selectedModel ? t(selectedModel.nameKey).split('(')[0].trim() : ''} Base</span>
                        <span className="text-warm-text font-bold text-sm w-max whitespace-nowrap">{selectedModel ? formatPrice(selectedModel.price) : '€0'}</span>
                     </div>
                     {selectedAddons.length > 0 && selectedAddons.map((opt, i) => (
                       <div key={i} className="flex justify-between items-start gap-4">
                         <span className="text-warm-muted text-sm leading-snug">{t(opt.nameKey)}</span>
                         <span className="text-warm-text font-medium text-sm w-max whitespace-nowrap">+{formatPrice(opt.price)}</span>
                       </div>
                     ))}
                  </div>
                  ...
                  <div className="border-t border-warm-border/50 pt-6 mb-8 relative z-10">
                     <div className="flex justify-between items-end">
                        <span className="text-gold text-xs font-label uppercase tracking-widest">{t('pricing.total_price')}</span>
                        <div className="text-right">
                          <span className="text-4xl font-black text-warm-text block leading-none mb-1">{formatPrice(totalPrice)}</span>
                          <span className="text-xs font-bold text-warm-muted">{t('pricing.excl_vat')}</span>
                        </div>
                     </div>
                  </div>

                  <Link to="/contact" className="gold-gradient text-white w-full py-4 rounded-xl font-bold text-sm md:text-base gold-glow hover:translate-y-[-2px] transition-all duration-300 flex items-center justify-center gap-2 relative z-10">
